...
st.title('Nhận diện bệnh thông qua lá cây')
st.write("Tải hình ảnh sâu bệnh trên lá cây")

# Setting the files that can be uploaded
uploaded_file = st.file_uploader("Tải hình ảnh lên", type=["png", "jpg"])

# If there is a uploaded file, start making prediction
if uploaded_file != None:
    
    # Display progress and text
    progress = st.text("Đang chẩn đoán phần trăm bệnh")
    ...
    # Show the results
    st.write(f"Lá cây {result['status']} với {result['prediction']} phát hiện được.")
